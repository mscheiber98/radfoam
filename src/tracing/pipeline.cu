...
__global__ void forward(TraceSettings settings,
    ...
    // additionally one set for the density
    // the number of coefficients per degree is calculated by (1 + degree)²
    constexpr int sh_dim_color = 3 * (1 + sh_degree) * (1 + sh_degree);
    constexpr int sh_dim_density = (1 + sh_degree) * (1 + sh_degree);
    constexpr int attr_memory_size = sh_dim_color + sh_dim_density;
    ...
    auto load_attributes = [&](uint32_t v_idx, Vec3f &rgb, float &s) {
        ...
        s = load_sh_as_density<attr_scalar, sh_degree>(sh_coeffs, attr_ptr + sh_dim_color);
...
__global__ void backward(TraceSettings settings,
    ...
    //lambda function to load vertex attributes
    auto load_attributes = [&](uint32_t v_idx, Vec3f &rgb, float &s, bool only_s = false) {
        const attr_scalar *attr_ptr = attributes + v_idx * attr_memory_size;
        s = load_sh_as_density<attr_scalar, sh_degree>(sh_coeffs, attr_ptr + sh_dim_color);
        //if density is larger than threshold, load color using spherical harmonics
        if (only_s) {
...
#pragma unroll
    ...
    for (uint32_t i = 0; i < num_depth_quantiles; ++i) {
        if (quantile_point_indices[thread_idx * num_depth_quantiles + i] !=
            ...
            Vec3f rgb;
            float s;
            load_attributes(point_idx, rgb, s, true);
            s = softplus(s);

...
//chunk size = how many cell faces are processed at once 
template <int block_size, int chunk_size, typename CellFunctor>
//inlining to reduce call overhead, since this function is called lots of times (each ray kernel calls it)
...
trace(const Ray &ray,
    ...
    // Outer loop that iterates through Voronoi cells
    for (;;) {
        n++;
        if (n > max_steps) {
            break;
        ...
        // number of faces of the current cell
        ...
        //temporarily stores adjacent point data
        //half2 
        half2 chunk[chunk_size * 2];
        // for each chunk
        for (uint32_t i = 0; i < num_faces; i += chunk_size) {
#pragma unroll
            //load chunk_size (4) adjacent points from global memory
...
#pragma unroll
            ...
            for (uint32_t j = 0; j < chunk_size; ++j) {
                ...
                //dot product of face normal and ray direction
                float dp = face_normal.dot(ray.direction);
                //distance along ray to intersection with plane
                float t = (face_origin - ray.origin).dot(face_normal) / dp;

                // keep the closest intersection that is not an intersection with a backface
                if (dp > 0.0f && t < t_1 && (i + j) < num_faces) {
                    // set the distance along the ray where the ray exits the cell
                    t_1 = t;
                    next_face = i + j;
                }
            }
        ...
        // the closest intersection also gives the next cell to step in
        uint32_t next_point_idx =
            point_adjacency[point_adjacency_begin + next_face];
        next_point = points[next_point_idx];

        // use the cell functor to calculate the contribution to the rendering equation of the current cell
        // break loop if remaining transmittance is too low (avoid endless tracing)
        if (t_1 > t_0) {
            if (!cell_functor(
        ...
        // prepare variables for the next cell
        t_0 = fmaxf(t_0, t_1);
